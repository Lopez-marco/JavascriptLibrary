/*
Comments 

1-grammarAndTypes
 01-comments-and-console.js
 */

 //value of a comment:
 //communicates to anyone readin our code

 //types of commnets
 //1) single-line comments\

 /*
2) multi-line comment
*/

//console.log
//outputs some message to the console
console.log("this is a console.log massage!");

console.log("you can separate parts of a console message with commas,","just like this!");

console.log(13,15,17,18);

//console.log and debbuging
//we can debug with console.log messages in 2 ways:
//1) we can check the status of some javascript 'thing'
//2) we can check a process we have built with repeated console.log messages

//example 1:
var sampleName = 'Marco';
console.log(sampleName);

//example 2:
var sampleNum = 1;
console.log(sampleNum);
sampleNum++;
console.log(sampleNum);
sampleNum++;
console.log(sampleNum);